...
```bash
cd discovery
docker build -t registry.example.com/platform/cluster-resource-report:0.3.1 .
docker push registry.example.com/platform/cluster-resource-report:0.3.1
...
helm upgrade --install resource-report charts/cluster-resource-report \
  ...
  --set image.tag=0.3.1          # or omit both to use ghcr.io/cdalar/cluster-resource-report:<appVersion>
```
